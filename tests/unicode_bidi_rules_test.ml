...
open Unicode_bidi_rules_extracted
...
let array_to_list arr =
  let rec aux acc i =
    if i < 0 then acc
    else aux (arr.(i) :: acc) (i - 1)
  in
  aux [] (Array.length arr - 1)

let compare_rule_result name total failed input reference_list extracted_result =
  incr total;
  if reference_list <> extracted_result then begin
    incr failed;
    Printf.printf "%s comparison failed:\n" name;
    Printf.printf "Input: %s\n" (string_of_bidi_class_list input);
    Printf.printf "Reference after %s: %s\n" name (string_of_bidi_class_list reference_list);
    Printf.printf "Extracted %s result: %s\n" name (string_of_bidi_class_list extracted_result);
    Printf.printf "\n";
    flush stdout;
    end

let compare_w1_result input reference_list w1_result =
  compare_rule_result "W1" w1_total w1_failed input reference_list w1_result

let compare_w2_result input reference_list w2_result =
  compare_rule_result "W2" w2_total w2_failed input reference_list w2_result

let compare_w3_result input reference_list w3_result =
  compare_rule_result "W3" w3_total w3_failed input reference_list w3_result

let compare_w4_result input reference_list w4_result =
  compare_rule_result "W4" w4_total w4_failed input reference_list w4_result

let compare_w5_result input reference_list w5_result =
  compare_rule_result "W5" w5_total w5_failed input reference_list w5_result

let compare_w6_result input reference_list w6_result =
  compare_rule_result "W6" w6_total w6_failed input reference_list w6_result
...
let resolve_weak_types (sequence: isolating_run_sequence) : unit =
  ...
  (* Save a copy of the original types (for input display) *)
  let original_input = array_to_list types in
  ...
  (* compare with extracted W1 *)
  let reference_after_w1 = array_to_list types in
  let sos = sequence.sos in
  let w1_result = rule_w1 original_input sos in
  compare_w1_result original_input reference_after_w1 w1_result;
  ...
  (* W2 comparison *)
  let reference_after_w2 = array_to_list types in
  let w2_result = rule_w2 reference_after_w1 false in
  compare_w2_result reference_after_w1 reference_after_w2 w2_result;
  ...
  (* W3 comparison *)
  let reference_after_w3 = array_to_list types in
  let w3_result = rule_w3 reference_after_w2 in
  compare_w3_result reference_after_w2 reference_after_w3 w3_result;
  ...
  (* W4 comparison *)
  let reference_after_w4 = array_to_list types in
  let prev_for_w4 = sequence.sos in
  let w4_result = rule_w4 reference_after_w3 prev_for_w4 in
  compare_w4_result reference_after_w3 reference_after_w4 w4_result;
  ...
  (* W5 comparison *)
  let reference_after_w5 = array_to_list types in
  let w5_result = rule_w5 reference_after_w4 false in
  compare_w5_result reference_after_w4 reference_after_w5 w5_result;
  ...
  (* W6 comparison *)
  let reference_after_w6 = array_to_list types in
  let w6_result = rule_w6 reference_after_w5 in
  compare_w6_result reference_after_w5 reference_after_w6 w6_result;
...
let run_tests test_cases =
  ...
  Printf.printf "Failed %d of %d w1 comparisons\n" !w1_failed !w1_total;
  Printf.printf "Failed %d of %d w2 comparisons\n" !w2_failed !w2_total;
  Printf.printf "Failed %d of %d w3 comparisons\n" !w3_failed !w3_total;
  Printf.printf "Failed %d of %d w4 comparisons\n" !w4_failed !w4_total;
...
(* end of unicode_bidi_rules_test.ml *)
