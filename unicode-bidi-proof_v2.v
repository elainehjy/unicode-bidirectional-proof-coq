...
Fixpoint rule_n1_before_strong (prev : bidi_class) (text : list bidi_class) : option bidi_class :=
  match text with
  | [] => None
  | c :: text' =>
      match c with
      | L => if (eqb_bidi_class prev L) then Some L else None
      | R | EN | AN => if (eqb_bidi_class prev R) || (eqb_bidi_class prev EN) || (eqb_bidi_class prev AN) then Some R else None
      | _ => rule_n1_before_strong prev text'
      end
  end.
...
Fixpoint rule_n1 (prev : bidi_class) (text : list bidi_class) : list bidi_class :=
  ...
  | c :: text' =>
      ...
      | B | S | WS | ON | FSI | LRI | RLI | PDI =>
          match rule_n1_before_strong prev text' with
          | Some L => if (eqb_bidi_class prev L) then L :: rule_n1 L text' else c :: rule_n1 c text'
          | Some R => if (eqb_bidi_class prev R) || (eqb_bidi_class prev EN) || (eqb_bidi_class prev AN)
                      then R :: rule_n1 R text'
...
Compute (rule_n1 sos [AN; ON; B; WS; R]).
...
(*
Fixpoint rule_w17_n1_before_strong (after_l after_en is_al : bool) (prev : bidi_class) (text : list bidi_class) : option bidi_class :=
  match prev, text with
  | _, [] => None
  | L, L :: _ => Some L
  | _, L :: _ => None
  ...
  | EN, AL :: _ => Some R
  | _,  AL :: _ => None
  | L,  EN :: _ => if is_al then None else (if after_l then (Some L) else None)    
  | R,  EN :: _ => if is_al then (Some R) else (if after_l then None else (Some R))                       
  | EN, EN :: _ => if is_al then (Some R) else (if after_l then None else (Some R))     
  | AN, EN :: _ => if is_al then (Some R) else (if after_l then None else (Some R))     
  | _,  EN :: _ => None
  | R,  AN :: _ => Some R
  | EN, AN :: _ => Some R
  | AN, AN :: _ => Some R
  | _,  AN :: _ => None
  | L, ET :: text' => if after_en then (Some L) else if rule_w15_before_en is_al ET text' then (if after_l then (Some L) else None) else None
  | R, ET :: text' => if after_en then None else if rule_w15_before_en is_al ET text' then (if after_l then (Some L) else None) else None
  (* if after_en
      then (if after_l then L else EN) :: rule_w17 after_l after_en is_al ET text'
      else if rule_w15_before_en is_al ET text'
           then (if after_l then L else EN) :: rule_w17 after_l after_en is_al ET text'
           else ON :: rule_w17 after_l after_en is_al ET text' *)
  | _, _ :: text' => rule_w17_n1_before_strong after_l false is_al prev text'
  end.  

Lemma rule_w17_n1_before_strong_correct: forall (text : list bidi_class) (after_l after_en is_al : bool) (prev : bidi_class),
    rule_n1_before_strong prev (rule_w17 after_l after_en is_al prev text) = rule_w17_n1_before_strong after_l after_en is_al prev text.
Proof.
  intro text.
  induction text as [ | c text' IH]; intros after_l after_en is_al prev.
  - destruct prev; reflexivity.
...
Fixpoint rule_n2 (level : nat) (text : list bidi_class) : list bidi_class :=
  match text with
  | [] => []
  | B :: text' | S :: text' | WS :: text' | ON :: text' | FSI :: text' | LRI :: text' | RLI :: text' | PDI :: text' =>
      if Nat.even level
      then L :: rule_n2 level text'
      else R :: rule_n2 level text'
  | c :: text' => c :: rule_n2 level text'
  end.

Compute (rule_n2 2 [L; ON; R]).
